...
exports.create_master_cloud_config = function (conf) {
  ...
  var output_file = util.join_output_file_path(conf.variables.cluster_name, 'master_node.yaml');
  var settings = conf.kubernetes;

  var write_files_pem = cloud_config.write_sensitive_files_from(['credentials', conf.variables.cluster_name, 'master'].join('/'), '/etc/kubernetes/ssl');
...
exports.create_node_cloud_config = function (conf) {
  ...
  var output_file = util.join_output_file_path(conf.variables.cluster_name, 'worker_node.yaml');
  var settings = conf.kubernetes;

  var write_files_pem = cloud_config.write_sensitive_files_from(['credentials', conf.variables.cluster_name, 'worker'].join('/'), '/etc/kubernetes/ssl');
  return cloud_config.process_template(input_file, output_file, function(data) {
    data = JSON.parse(_.template(JSON.stringify(data))(settings)); // substitude template variables
