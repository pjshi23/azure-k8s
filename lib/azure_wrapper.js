...
var create_ssh_key = function (prefix) {
  var opts = {
    ...
    nodes: true,
    newkey: 'rsa:2048',
    subj: '/CN=' + conf.cluster_name + '.node',
    keyout: util.join_file_path('./credentials/' + conf.cluster_name, prefix, 'ssh.key'),
    out: util.join_file_path('./credentials/' + conf.cluster_name, prefix, 'ssh.pem'),
  };
...
var create_tls_config = function (conf) {
  var files = [
...
exports.create_cluster_config = function (config_file) {
  conf = process_cluster_config(config_file); 
  conf.resources.ssh_key = create_ssh_key(conf.cluster_name);
  hosts.ssh_port_counter = conf.nodes.starting_ssh_port;

  // setup tls assets
  create_tls_config(conf);
};
