...
var save_template = function (template_obj) {
  var file_name = util.join_output_file_path(conf.resources.cluster_name, 'deployment.json');
  try {
...
var create_tls_assets = function (conf) {
  ...
  try {
    for (f of files) {
      ...
      for (var n of conf.kubernetes.credentials[f.alt_names]) {
        data = data.concat(n + "\n");
      }
...
var process_cluster_template = function (template_file, conf) {
  ...
  obj.variables.imageSku = conf.resources.coreos_image_version.selected_channel;
  obj.variables.imageVersion = conf.resources.coreos_image_version[obj.variables.imageSku];
  // add expose ports
  ...
  obj.parameters.masterCustomData.defaultValue = fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'master_node.yaml')).toString();
  obj.parameters.workerCustomData.defaultValue = fs.readFileSync(util.join_output_file_path(conf.resources.cluster_name, 'worker_node.yaml')).toString();

  obj.parameters.sshKeyData.defaultValue = fs.readFileSync(util.join_file_path('./credentials/' + conf.resources.cluster_name, conf.resources.cluster_name, 'ssh.pub')).toString();
...
exports.create_cluster_template = function (config_file) {
  ...
  fs.createReadStream('./deployment/scaling-template.json').pipe(
    fs.createWriteStream(util.join_output_file_path(conf.resources.cluster_name, 'scaling.json')));
};
